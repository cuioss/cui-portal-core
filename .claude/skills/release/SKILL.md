---
name: release
description: Cut a cui-portal-core release — bump .github/project.yml version, open and merge the release PR, wait for the automated Release workflow, verify the release landed, then reformat the generated GitHub release notes
user-invocable: true
...
Cuts a new cui-portal-core release end-to-end: determine the version, open the version-bump PR
...
- `release.current-version` (currently `1.4.0`) — the **last released** version.
- `release.next-version` (currently `1.5-SNAPSHOT`) — what `pom.xml` carries between releases.

**The choice is genuinely open here** and the skill must not guess:
`current-version` is `1.4.0` but the pom floats on `1.5-SNAPSHOT`, so `current-version` does
**not** sit on the `next-version` line.

- **`1.5`** — strip `-SNAPSHOT`, starting the new minor line; `next-version` then
  moves to the following minor.
- **`1.4.1`** — continue the patch line; `next-version` stays `1.5-SNAPSHOT`.
...
```bash
gh pr list --repo cuioss/cui-portal-core --state open --json number,title,isDraft
```
- **No open PRs** → proceed.
...
```bash
git checkout -b chore/release_1.5
```
...
- `next-version:` → for a new minor line, the following minor + `-SNAPSHOT`; for a patch
  release, **leave unchanged** (`1.5-SNAPSHOT`)
...
if ! gh label list --repo cuioss/cui-portal-core --search skip-bot-review --json name --jq '.[].name' \
     | grep -qx skip-bot-review; then
  gh label create skip-bot-review --repo cuioss/cui-portal-core \
    --description "Skip automated bot review" --color ededed
fi
gh pr create --repo cuioss/cui-portal-core --base main \
...
```bash
gh pr checks <pr#> --repo cuioss/cui-portal-core --watch
```
...
- Every review comment MUST get a reply and MUST be resolved — fix it and say so, or explain
  ...
  ```bash
  gh api --paginate repos/cuioss/cui-portal-core/pulls/<pr#>/comments
  ```
- **Unresolved review threads block the merge.** A PR can show every check green and still
  ...
  ```bash
  gh api graphql --paginate -f query='query($endCursor:String){repository(owner:"cuioss",name:"cui-portal-core"){pullRequest(number:<pr#>){reviewThreads(first:100,after:$endCursor){pageInfo{hasNextPage endCursor} nodes{id isResolved}}}}}'
...
```bash
gh pr merge <pr#> --repo cuioss/cui-portal-core --squash --delete-branch
```
...
```bash
gh run list --repo cuioss/cui-portal-core --workflow "Release" --limit 3 \
  --json status,conclusion,displayTitle,databaseId
gh run watch <databaseId> --repo cuioss/cui-portal-core
```

### Step 11 — Verify the release landed

```bash
gh release view <version> --repo cuioss/cui-portal-core --json tagName,name,isDraft,createdAt
...
```bash
mkdir -p .plan/temp
gh release view <version> --repo cuioss/cui-portal-core --json body --jq .body \
  > .plan/temp/release-<version>-orig.md
# ...build the reformatted body in .plan/temp/release-<version>.md...
gh release edit <version> --repo cuioss/cui-portal-core --notes-file .plan/temp/release-<version>.md
```
