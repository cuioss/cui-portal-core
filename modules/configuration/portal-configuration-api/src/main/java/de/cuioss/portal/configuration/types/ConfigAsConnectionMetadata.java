...
/**
 * <p>
 * Injects a number of related config-properties as a {@link ConnectionMetadata}. In case the
 * required elements are not there / empty the producer will throw an
 * {@link IllegalArgumentException}, see {@link #failOnInvalidConfiguration()}.
 * <p>
 * 'baseName' is the string that is equal for all other derived keys, e.g. for the
 * value-set-client:
 * 'value-set-client.connection.url':
 * 'value-set-client.connection.' is the 'baseName' for
 ...
 * <li><code>baseName.description</code>: Optional, description for that connection.</li>
 ...
 * <li><code>baseName.authentication.certificate.keystore.keypassword</code> (To be used for
 ...
 * </ul>
 * <p>
 * In case of authentication type "token.application" the following properties must be defined:
 * </p>
 * <ul>
 * <li><code>baseName.authentication.token.application.token</code>: Identifying the actual token to
 * be used. Usually a client token</li>
 ...
 * </p>
 * <ul>
 * <li><code>baseName.authentication.token.user.token.source</code>: Identifies the Information from
 * the AuthenticatedUserInfo that is used as source for the token</li>
 ...
 * <li><code>baseName.transport.secure.keystore.keypassword</code> (To be used for
 * cases where the actual certificate has a different password compared to the
 * {@code keystore.password})</li>
 ...
 * <li><code>baseName.transport.secure.disableHostNameVerification</code>: If set to {@code true}
 ...
 * provides a built-in caching: Http code 304 will always handled correctly. In addition the caching
 * can be configured in many aspects, see other parameter.
 * </li>
 * <li>{@code baseName.config.max-age}: Sets the maximum age of a cached response in
 * minutes. If the cache response's age exceeds maxAge, it will not be used and a network request
 * will be made.</li>
 * <li>{@code baseName.config.max-stale}: Accept cached responses in
 * minutes that have exceeded their freshness lifetime by up to maxStale. If unspecified, stale
 * cache responses will not be used.</li>
...
public @interface ConfigAsConnectionMetadata {
     ...
     *         {@link ConnectionMetadata} without calling {@link ConnectionMetadata#validate()}
     */
    @Nonbinding
